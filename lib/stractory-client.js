    ...
    var c = net.connect(hostport, function() {
        ...
        d.once('remote', function(r) {
            ...
            var passWorkerEvents = function(cli, name, w) {
                cli.on('timeout', function() { self.emit('worker-timeout', name, w); });
                cli.on('error', function(err) { self.emit('worker-error', name, w, err); });
                cli.on('fail', function(err) { self.emit('worker-fail', name, w, err); });
                cli.on('close', function(had_err) { self.emit('worker-close', name, w, had_err); });
            }
